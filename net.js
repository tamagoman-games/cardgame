...
const NET = {
  mode: 'local',   // 'local'(通常のAI対戦) | 'host' | 'guest' | 'spectator'
  peer: null,
  conn: null,
  myDeckIds: null,
  guestDeckIds: null,
  spectatorConns: []  // ホストのみ使用。3人目以降が同じコードで参加してきた時の観戦用接続一覧
};
...
function pvpResetToTitle(message){
  ...
  (NET.spectatorConns || []).forEach(c => { try { c.close(); } catch(e){} });
...
function pvpConnect(rawCode){
  ...
  hostPeer.on('open', () => {
    ...
    hostPeer.on('connection', conn => {
      if (NET.conn){
        // 既に対戦相手(NET.conn)が決まっている状態で3人目以降が同じコードで
        // 参加してきた場合。対戦相手を上書きしてしまうと2人目が置いてけぼりに
        // なってしまうため、この接続は観戦者として扱う。
        pvpSetupSpectatorConn(conn);
      }else{
        NET.conn = conn;
...
function pvpSetupConn(conn, role){
  conn.on('open', () => {
    ...
    // 【バグ修正】ここが常にDEFAULT_DECK_COUNTSで初期化されており、保存済みデッキがあっても
    // 対人戦では読み込まれず、AI対戦と違って毎回おまかせ編成からになっていた。
    // AI対戦(btn-start)と同じく、最後に使った保存済みデッキがあればそこから始めるように修正。
...
function hostHandleGuestMessage(msg){
  ...
  else if (msg.t === 'spell') pvpEnemySpell(msg.handUid, msg.side, msg.slot, msg.sacrifice);
...
// ホストの盤面(state)から、ゲスト/観戦者へ送る視点データを組み立てる。
// ゲストへの送信・観戦者への送信の両方で同じ内容を使い回す。
function buildHostView(){
  if (!state.enemy || !state.player) return null;
  return {
    turn: state.turn,
    phase: state.phase,
...
function broadcastView(){
  ...
  if (!view) return;
  if (NET.conn && NET.conn.open) netSend({ t: 'view', v: view });
  // 観戦者にも同じ盤面を一方的に送る(観戦者からの操作は受け付けない)
  (NET.spectatorConns || []).forEach(c => {
    if (c.open){
      try{ c.send({ t: 'view', v: view }); }catch(e){}
    }
  });
}

// 3人目以降が同じコードで参加してきた時の接続処理。
// 対戦相手(NET.conn)は既に決まっているので盤面の更新だけを一方的に送り、
// この接続からの操作(data)は一切受け付けない。
function pvpSetupSpectatorConn(conn){
  conn.on('open', () => {
    NET.spectatorConns.push(conn);
    conn.send({ t: 'spectator' });
    const view = buildHostView();
    if (view) conn.send({ t: 'view', v: view });
  });
  conn.on('data', () => {}); // 観戦者からの操作意図は無視する
  conn.on('close', () => {
...
function pvpEnemySummon(slot, handUid, sacrificeSlots){
  ...
  const actualLeaving = validSacs.filter(s => !(hasAbility(state.enemy.field[s],'不死') && !state.enemy.field[s].revived)).length;
  const projected = fieldCount(state.enemy) - actualLeaving;
...
function pvpEnemySpell(handUid, side, slot, sacrificeSlots){
  if (state.active !== 'enemy' || state.locked || state.phase !== 'main' || state.pendingSkeleton) return;
  const idx = state.enemy.hand.findIndex(c => c.uid === handUid);
  if (idx === -1) return;
  const card = state.enemy.hand[idx];
...
function guestHandleHostMessage(msg){
  if (!msg || !msg.t) return;
  if (msg.t === 'spectator') { pvpEnterSpectatorMode(); return; }
  if (msg.t === 'view') guestApplyView(msg.v);
}

// ホストから「対戦は既に進行中なので観戦者として参加した」と通知された時の処理。
// デッキ編成はスキップし、盤面を見るだけ(操作不可)の状態にする。
function pvpEnterSpectatorMode(){
  NET.mode = 'spectator';
  pvpStatus('観戦者として参加しました(対戦は他の2人が進行中です)');
  toast('対戦は既に進行中のため、観戦者として参加しました');
