  ...
  while(changed && guard++ < 10){
    ...
    for(let i=0;i<state.enemy.hand.length;i++){
      ...
      if(card.effect==='soulGain'){
        ...
        let sacPick = null;
        if(card.sacrifice>0){
          sacPick = pickSacrificesForAI(state.enemy, -1, card.sacrifice);
          if(!sacPick.enough) continue; // 生け贄にできるモンスターがいなければ使わない
        }
        if(sacPick){
          const names = [];
          sacPick.chosen.forEach(({index})=>{
            const sacCard = state.enemy.field[index];
            if(sacCard){
              if(hasAbility(sacCard,'不死') && !sacCard.revived){
                sacCard.currentHp = sacCard.hp;
                sacCard.revived = true;
                log(`敵が「${sacCard.name}」を生け贄に捧げたが、不死が発動して場に残った`);
              }else{
                state.enemy.field[index]=null;
                state.enemy.grave.push(sacCard);
                const amt = gainSoulOnDeath(sacCard, state.enemy);
                triggerOnDeath(sacCard, state.enemy, index);
