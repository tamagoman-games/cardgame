const state = {
  ...
  pendingSkeleton: false // ネクロマンサーのスケルトンをどこに出すか選んでいる最中かどうか
};
    ...
    const cells = Array.from({length:FIELD_LANES},(_,lane)=>{
      ...
      if(!card){
        ...
        if(!enemy && row.isBack){
          ...
          let label = '空き(召喚エリア)';
          if(state.pendingSkeleton) label = 'スケルトンをここに召喚';
...
function renderHand(){
  ...
  document.querySelectorAll('[data-hand]').forEach(el=>{
    el.onclick=()=>{
      ...
      if(state.pendingSkeleton){ toast('先にスケルトンの召喚場所を選んでください'); return; }
...
function render(){
  ...
  $('btn-draw').disabled = state.phase!=='draw' || state.locked || hostOffTurn || state.pendingSkeleton;
  $('btn-scavenge').disabled = state.phase!=='draw' || state.locked || hostOffTurn || state.pendingSkeleton;
  $('btn-end').disabled = state.phase!=='main' || state.locked || hostOffTurn || state.pendingSkeleton;
...
function drawCard(){
  if(NET.mode==='guest'){
    if(state.locked || state.phase!=='draw') return;
    netSend({t:'draw'});
    return;
  }
  if(state.locked) return;
    ...
    switch(ability){
      case '召喚時': {
        if(card.id==='necromancer' && fieldCount(owner)<MAX_FIELD_CARDS){
          ...
          if(emptySlots.length>1 && owner===state.player){
            // 空きが複数あるプレイヤー本人の場合は、出す場所を選べるようにする
            state.pendingSkeleton = true;
            toast('スケルトンを召喚する場所を選んでください');
            log('あなたの「ネクロマンサー」がスケルトンを呼び出そうとしている(空きマスを選択)');
      ...
      case '知識': {
        if(owner.deck.length>0 && owner.hand.length<5){
...
function endTurn(){
  if(NET.mode==='guest'){
    if(state.locked || state.phase!=='main') return;
    netSend({t:'endTurn'});
    return;
  }
  if(state.locked) return;
