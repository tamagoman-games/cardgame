...
// 「不死」を持ち、まだ一度も復活していないカードを生け贄に選んでも、実際には死なずに
// 満タンHPで復活するだけで生け贄コストを支払ったことにはならない(値は0)。
// 一度「不死」を使い切った後(revived===true)のカードは、通常どおり生け贄として機能する。
function sacrificeUnitValue(card){
  if(hasAbility(card, '不死') && !card.revived) return 0;
  return hasAbility(card, '供物') ? 2 : 1;
}
// 生け贄に選んだカードのうち、実際に場から離れて生け贄になる(=不死で復活しない)枚数。
// 場の4体制限の判定で「本当に空くマス数」を数えるために使う。
...
function onPlayerLane(slot){
  ...
  // 「不死」で復活して場に残るカードは実際には空きマスを作らないので、
    ...
    state.sacrificeTargets.forEach(slot=>{
      ...
      if(sacCard){
        if(hasAbility(sacCard, '不死') && !sacCard.revived){
          ...
          log(`「${sacCard.name}」は生け贄に選ばれたが、不死で復活し生け贄にはならなかった`);
        }else{
...
// 場は合計4体まで、ドロー/死体を漁るはどちらか1回、手札上限8、突進なし、など)は
  ...
  while(safety++ < 20){
    ...
    for(let i=0;i<state.enemy.hand.length;i++){
      ...
      for(const target of emptySlots){
        ...
        // 「不死」で復活して場に残るカードはマスを空けないので、実際に離脱する枚数だけを引く。
    ...
    if(card.sacrifice>0){
      ...
      pick.chosen.forEach(item=>{
        if(hasAbility(item.c, '不死') && !item.c.revived){
          ...
          item.c.revived = true;
          log(`敵の「${item.c.name}」は生け贄に選ばれたが、不死で復活し生け贄にはならなかった`);
        }else{
