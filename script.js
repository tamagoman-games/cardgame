const state = {
  ...
  sacrificeTargets: [],
  deckCounts: {},
  currentDeckName: '', // デッキ編成画面で選択中/保存先の名前('' は「新規デッキ」扱い)
  player: null,
  enemy: null,
...
// 【機能拡張】以前は1つしかデッキを保存できなかったが、名前を付けて複数保存し、
// 対戦前(AI対戦・対人戦どちらでも)に選べるようにしてほしいと依頼されたため、
// 単一デッキ用のキーから「名前付きデッキのリスト」形式に拡張した。
// 旧形式(soulSacrifice_savedDeck に単一デッキだけが入っている状態)からは自動で移行する。
const DECK_STORAGE_KEY_LEGACY = 'soulSacrifice_savedDeck';
const DECKS_STORAGE_KEY = 'soulSacrifice_savedDecksV2';

// {decks:[{name, counts}], lastUsed: name|null} を返す
function loadDeckStore(){
  ...
  }catch(e){ /* 壊れたデータは無視して下の移行/初期化に進む */ }

  // 新形式が無ければ、旧形式(単一デッキ)からの移行を試みる
  try{
    const legacyRaw = localStorage.getItem(DECK_STORAGE_KEY_LEGACY);
    if(legacyRaw){
      const legacyCounts = JSON.parse(legacyRaw);
      if(legacyCounts && typeof legacyCounts === 'object'){
        const migrated = { decks:[{ name:'保存したデッキ', counts: legacyCounts }], lastUsed:'保存したデッキ' };
        saveDeckStore(migrated);
        return migrated;
      }
    }
  }catch(e){ /* 移行に失敗しても致命的ではないので空で始める */ }

  return { decks:[], lastUsed:null };
}

function saveDeckStore(store){
  try{
    localStorage.setItem(DECKS_STORAGE_KEY, JSON.stringify(store));
    return true;
  }catch(e){
    return false;
  }
...
// 名前が既に存在すれば上書き、無ければ新規追加。最後に使ったデッキとしても記録する
function upsertSavedDeck(name, counts){
  const store = loadDeckStore();
  const idx = store.decks.findIndex(d=>d.name===name);
  const entry = { name, counts:{...counts} };
  if(idx>=0) store.decks[idx] = entry;
  else store.decks.push(entry);
  store.lastUsed = name;
  return saveDeckStore(store) ? store : null;
}

function deleteSavedDeck(name){
  const store = loadDeckStore();
  store.decks = store.decks.filter(d=>d.name!==name);
  if(store.lastUsed===name){
    store.lastUsed = store.decks.length ? store.decks[0].name : null;
  }
  saveDeckStore(store);
  return store;
}
...
function getLastUsedDeck(){
  const store = loadDeckStore();
  if(!store.lastUsed) return null;
  return store.decks.find(d=>d.name===store.lastUsed) || null;
}
...
// 保存済みデッキの一覧をプルダウンに反映する(枚数編集のたびに呼ばれるrenderDeckBuilderからは
// 中身が変わらないので毎回作り直しても軽い)
function renderDeckSelect(){
  const store = loadDeckStore();
  const select = $('deck-select');
  const options = ['<option value="">— 新規デッキ —</option>']
    .concat(store.decks.map(d=>`<option value="${d.name}">${d.name}</option>`));
  select.innerHTML = options.join('');
  select.value = state.currentDeckName || '';
  $('deck-name-input').value = state.currentDeckName || '';
  $('btn-deck-delete').disabled = !state.currentDeckName || !store.decks.some(d=>d.name===state.currentDeckName);
}
...
function renderHand(){
  ...
  document.querySelectorAll('[data-hand]').forEach(el=>{
    el.onclick=()=>{
      if(NET.mode==='spectator') return; // 観戦者は操作できない
      ...
      // 対象を選ばない魔法(魂の奔流など)は、生け贄が不要ならタップした瞬間に即発動する。
      // 生け贄が必要な場合は、通常のモンスター召喚と同じく先に生け贄を選んでもらう。
...
function render(){
  ...
  const enemyLabel = (NET.mode==='host' || NET.mode==='guest' || NET.mode==='spectator') ? '相手の番' : '敵の番';
  ...
  const isSpectator = NET.mode==='spectator'; // 観戦者はボタン操作も一切不可
  $('btn-draw').disabled = isSpectator || state.phase!=='draw' || state.locked || hostOffTurn || state.pendingSkeleton==='player';
  $('btn-scavenge').disabled = isSpectator || state.phase!=='draw' || state.locked || hostOffTurn || state.pendingSkeleton==='player';
  $('btn-end').disabled = isSpectator || state.phase!=='main' || state.locked || hostOffTurn || state.pendingSkeleton==='player';
...
function onPlayerLane(slot){
  ...
  if(card.type==='spell'){
    // 対象不要だが生け贄が必要な魔法(魂の奔流など)は、通常のモンスター召喚と同じ
    // 「自分の場のモンスターをタップして生け贄に選ぶ」操作にする。
    if(card.target==='none' && card.sacrifice>0){
      if(!state.player.field[slot]) return; // 何もいないマスは無視
      ...
      if(sacrificeValue(state.sacrificeTargets, state.player)>=card.sacrifice){
        tryPlayerCastSpell(card, idx, 'none', -1);
      }else{
        render();
      }
...
function tryPlayerCastSpell(card, idx, side, slot){
  ...
  if(NET.mode==='guest'){
    netSend({t:'spell', handUid:card.uid, side, slot: (typeof slot==='number'?slot:-1), sacrifice:[...state.sacrificeTargets]});
    state.selectedHand=null;
    state.sacrificeTargets=[];
    render();
    return true;
  ...
  if(card.sacrifice>0){
    const sacrificedNames = [];
    state.sacrificeTargets.forEach(s=>{
      const sacCard = state.player.field[s];
      if(sacCard){
        if(hasAbility(sacCard, '不死') && !sacCard.revived){
          sacCard.currentHp = sacCard.hp;
          sacCard.revived = true;
          ...
          state.player.field[s]=null;
          state.player.grave.push(sacCard);
          const soulAmount = gainSoulOnDeath(sacCard, state.player);
          triggerOnDeath(sacCard, state.player, s);
          sacrificedNames.push(sacCard.name);
          log(`生け贄「${sacCard.name}」で魂+${soulAmount}`);
        }
      }
  ...
  card.abilities.forEach(ability=>{
    switch(ability){
      case '召喚時': {
        ...
        if(card.id==='demon'){
          // 相手モンスターの中で最も強力な1体を無条件で破壊する(装甲・守護・毒無効などの
          // 耐性は一切貫通する、別枠の「破壊」効果。戦闘由来の「毒」即死とは異なる)
          const opp = (owner===state.player) ? state.enemy : state.player;
          const targets = fieldSlots(opp);
          if(targets.length>0){
            targets.sort((a,b)=>cardPower(b.c)-cardPower(a.c));
            const {c: victim, index: victimIndex} = targets[0];
            opp.field[victimIndex] = null;
            opp.grave.push(victim);
            const soulAmount = gainSoulOnDeath(victim, opp);
            triggerOnDeath(victim, opp, victimIndex);
            log(`${owner===state.player?'あなた':'敵'}の「デーモン」が「${victim.name}」を破壊した(${opp===state.player?'あなた':'敵'}は魂+${soulAmount})`);
            playSound('sacrifice');
          }
        }
        break;
      ...
      case '速攻': {
        // 召喚された直後のこの時点ではまだ2列目にいるはず。1列目(同じレーン)が
        // 空いていれば、電光石火(瞬足)と同じ処理でそのまま1列目へ移動させる。
        if(rowOf(slot)==='back'){
          const lane = laneOf(slot);
          const front = frontIndex(lane);
          if(!owner.field[front]){
            owner.field[front] = card;
            owner.field[slot] = null;
            card.summonedThisTurn = false;
            card._summonedAt = Date.now();
            log(`${owner===state.player?'あなた':'敵'}の「${card.name}」は「速攻」で1列目へ移動し、すぐに攻撃可能になった`);
          }
        }
        break;
      }
...
function triggerOnDeath(card, owner, lane){
  ...
  card.abilities.forEach(ability=>{
    switch(ability){
      ...
      case '献血': {
        if(card.onDeath && owner.hand.length<HAND_LIMIT){
          const gained = makeCard(card.onDeath);
          owner.hand.push(gained);
          log(`${owner===state.player?'あなた':'敵'}の「${card.name}」の「献血」で「${gained.name}」を手札に加えた`);
        }
        break;
      }
      default:
...
//
// 実装方針:
//  1. chooseDrawOrScavenge      : ドロー / 死体を漁る のどちらが良いかを状況から判断
...
//
// 【リファクタリング】AI強化のため、Node.js上でAI同士を大量に自己対戦させて
// 統計を取れるようにしたいという依頼があり、chooseDrawOrScavenge/evaluateSummonOption/
// aiMainPhase/aiTryCastSpells/aiDrawPhaseを「state.enemy/state.player固定」から
// 「self(判断する側)/opp(相手)を引数で受け取る」形に一般化した(元は enemyChooseDrawOrScavenge
// のように名前も"enemy"固定だった)。挙動は従来と完全に同一(通常の敵AI戦は
// self=state.enemy, opp=state.player, selfLabel='敵'で呼ぶだけ)。この一般化により、
// 同じロジックをstate.player側にも使い回せるようになり、ゲーム本体に含めない
// 裏側専用のシミュレーションスクリプトから両陣営をAIに操作させて自己対戦を回せる。
// 自己対戦シミュレーションで実際に見つかった問題(AI_DECK_COUNTSの合計がプレイヤーの
// デッキ上限[DECK_MAX_SIZE=27]を超えて37枚になっていた)はcards.js側で修正済み。
...
function cardPower(card){
  ...
  if(hasAbility(card,'毒無効')) v += 1; // 相手の毒(即死)を無効化できるので場持ちが良い
  ...
  if(hasAbility(card,'献血')) v += 0.5;     // 死ぬと血の小瓶(次の生け贄要員)が手に入る。断末魔ほどではないが無駄死にしにくい
  return v;
}
...
// self: 判断する側(state.enemy または state.player)。AI vs AIの自己対戦シミュレーションでも
// 同じ関数をどちらの陣営にも使い回せるよう、state.enemy固定ではなく引数で受け取る。
function chooseDrawOrScavenge(self){
  if(self.hand.length >= HAND_LIMIT) return 'scavenge'; // 手札上限を超えてドローはできない
  if(self.deck.length === 0) return 'scavenge'; // 山札切れ(=敗北)を避ける

  const soul = self.soul;
  const hand = self.hand;
  const hasEmptySlot = self.field.some((c,idx)=>!c && rowOf(idx)==='back');
...
// selfLabel: ログ表示用の呼び名(通常のAI戦では'敵'、自己対戦シミュレーションでは任意のラベル)
function aiDrawPhase(self, selfLabel){
  const choice = chooseDrawOrScavenge(self);

  if(choice==='draw'){
    if(self.deck.length===0){
      return { deckout:true }; // 保険(chooseDrawOrScavengeで基本的に回避済み)
    }
    self.hand.push(self.deck.pop());
    log(`${selfLabel}が1枚ドロー`);
  }else{
    self.soul += 1;
    log(`${selfLabel}が死体を漁って魂を1獲得`);
  }
...
// self: 召喚を検討している側、opp: その相手。state.enemy/state.player固定ではなく引数化することで、
// 自己対戦シミュレーションでも同じ評価関数をどちらの陣営にも使い回せるようにしている。
function evaluateSummonOption(self, opp, card, lane){
  let score = card.atk + card.hp*0.5;

  const enemyFront = opp.field[frontIndex(lane)]; // selfから見た「相手」の正面
  const ownFieldCountNow = fieldCount(self);

  if(hasAbility(card,'守護')){
    const flyingLanes = flyingThreatLanes(self, opp);
  ...
  if(hasAbility(card,'身代わり')){
    ...
    if(hasAbility(card,'守護') && flyingThreatLanes(self, opp).length){
      score += 3; // 守護も併せ持つなら、飛行まで全レーンでブロックできる
    }
  }

  if(hasAbility(card,'毒')){
    if(enemyFront && hasAbility(enemyFront,'毒無効')){
      // 毒無効持ちには即死が通らないので過大評価しない
    }else if(enemyFront && enemyFront.currentHp >= 3){
  ...
  if(hasAbility(card,'飛行')){
    ...
    const globalFlyingBlock = frontRowAbilityHolder(opp, '身代わり', true);
  ...
  if(hasAbility(card,'毒無効')) score += 1;   // 相手の毒持ちに対して安心して壁にできる
  ...
  if(opp.hp <= 6) score += card.atk*0.5; // 相手の残りHPが少ない時は火力を重視
  if(self.hp <= 8 && hasAbility(card,'守護')) score += 1.5; // 自分が押されている時は守護を優先

  // 自分のHPが低い時、または相手が強いモンスター(攻撃力が高い・能力が強力)を出している時は、
  // 殴り合いより除去を最優先にする
  if(enemyFront){
    const selfPressured = self.hp <= 10;
    const bigThreat = cardPower(enemyFront) >= 5; // 攻撃力or能力込みで強い相手
    if(selfPressured || bigThreat){
      const wouldKill = (hasAbility(card,'毒') && !hasAbility(enemyFront,'毒無効')) ? card.atk>0 : card.atk>=enemyFront.currentHp;
...
// self: 行動する側、opp: その相手、selfLabel: ログ表示用の呼び名。
// 通常の敵AI戦ではself=state.enemy/opp=state.player/selfLabel='敵'で呼ぶ(挙動は従来と同じ)。
// 自己対戦シミュレーションではself/oppを入れ替えて呼ぶことで、同じロジックをプレイヤー側にも使い回せる。
function aiMainPhase(self, opp, selfLabel){
  const drawResult = aiDrawPhase(self, selfLabel);
  ...
  while(safety++ < 20){
    ...
    for(let i=0;i<self.hand.length;i++){
      ...
      if(c.soul > self.soul) continue;

      for(const target of emptySlots){
        const lane = laneOf(target.index);
        const pick = pickSacrificesForAI(self, target.index, c.sacrifice);
        ...
        const projectedCount = fieldCount(self) - actualLeaving + 1;
        if(projectedCount > MAX_FIELD_CARDS) continue;

        let score = evaluateSummonOption(self, opp, c, lane);
        if(c.id==='necromancer' && projectedCount < MAX_FIELD_CARDS) score += 1.5; // スケルトンも一緒に出せる
        if(c.id==='demon'){
          // 相手の場に強いモンスターがいるほど、召喚時の強制破壊が刺さる
          const oppTargets = fieldSlots(opp);
          if(oppTargets.length>0){
            const bestOppPower = Math.max(...oppTargets.map(t=>cardPower(t.c)));
            score += Math.min(4, 1 + bestOppPower*0.4);
          }
        }
    ...
    if(card.sacrifice>0){
      ...
      pick.chosen.forEach(item=>{
        if(hasAbility(item.c, '不死') && !item.c.revived){
          ...
          item.c.revived = true;
          log(`${selfLabel}が「${item.c.name}」を生け贄に捧げたが、不死が発動して場に残った`);
        }else{
          self.field[item.index]=null;
          self.grave.push(item.c);
          const soulAmount = gainSoulOnDeath(item.c, self);
          triggerOnDeath(item.c, self, item.index);
          names.push(item.c.name);
          log(`${selfLabel}の生け贄「${item.c.name}」で魂+${soulAmount}`);
        }
      });
      if(names.length) log(`${selfLabel}が「${names.join('」「')}」を生け贄に捧げた`);
    }
    ...
    log(`${selfLabel}が「${card.name}」を${rowOf(slot)==='back'?'2列目':'1列目'}に召喚`);
    playSound('summon');
    triggerOnSummon(card,self,slot);
  }

  aiTryCastSpells(self, opp, selfLabel);
...
function aiTryCastSpells(self, opp, selfLabel){
  let changed = true;
  let guard = 0;
  while(changed && guard++ < 10){
    ...
    for(let i=0;i<self.hand.length;i++){
      ...
      if(card.soul > self.soul) continue;

      if(card.effect==='damage'){
        // このダメージで倒せる、なるべく強い相手のモンスターを狙う
        let bestSlot=-1, bestPower=-1;
        fieldSlots(opp).forEach(({c,index})=>{
          let dmg = card.value;
        ...
        if(applySpellEffect(card, self, opp, bestSlot)){
          self.soul -= card.soul;
      ...
      if(card.effect==='buff'){
        ...
        const slots = fieldSlots(self);
        if(!slots.length) continue;
        slots.sort((a,b)=>cardPower(b.c)-cardPower(a.c));
        if(applySpellEffect(card, self, opp, slots[0].index)){
          self.soul -= card.soul;
          self.hand.splice(i,1);
          changed = true; break;
        }
      ...
      if(card.effect==='rush'){
        ...
        const candidates = fieldSlots(self).filter(({index})=>{
          if(rowOf(index)!=='back') return false;
          return !self.field[frontIndex(laneOf(index))];
        });
        if(!candidates.length) continue;
        candidates.sort((a,b)=>cardPower(b.c)-cardPower(a.c));
        if(applySpellEffect(card, self, opp, candidates[0].index)){
          self.soul -= card.soul;
          self.hand.splice(i,1);
          changed = true; break;
        }
      ...
      if(card.effect==='soulGain'){
        ...
        const canSummonSomething = self.hand.some(c=>c.type!=='spell' && c.soul<=self.soul);
        if(canSummonSomething) continue;
        let sacPick = null;
        if(card.sacrifice>0){
          sacPick = pickSacrificesForAI(self, -1, card.sacrifice);
          if(!sacPick.enough) continue; // 生け贄にできるモンスターがいなければ使わない
        }
        if(sacPick){
          const names = [];
          sacPick.chosen.forEach(({index})=>{
            const sacCard = self.field[index];
            if(sacCard){
              if(hasAbility(sacCard,'不死') && !sacCard.revived){
                sacCard.currentHp = sacCard.hp;
                sacCard.revived = true;
                log(`${selfLabel}が「${sacCard.name}」を生け贄に捧げたが、不死が発動して場に残った`);
              }else{
                self.field[index]=null;
                self.grave.push(sacCard);
                const amt = gainSoulOnDeath(sacCard, self);
                triggerOnDeath(sacCard, self, index);
                names.push(sacCard.name);
                log(`${selfLabel}の生け贄「${sacCard.name}」で魂+${amt}`);
              }
            }
          });
          if(names.length) log(`${selfLabel}が「${names.join('」「')}」を生け贄に捧げた`);
        ...
        if(applySpellEffect(card, self, opp, -1)){
          self.soul -= card.soul;
          self.hand.splice(i,1);
          changed = true; break;
        }
...
function runEnemyTurn(){
  ...
  const result=aiMainPhase(state.enemy, state.player, '敵');
...
function battle(atk, atkOwner, def, defOwner, slot){
  ...
  if(hasAbility(atk, '毒') && atkDamage > 0 && !hasAbility(def, '毒無効')){
    def.currentHp = 0;
  }else{
...
$('btn-start').onclick = () => {
  initAudio();
  NET.mode = 'local';
  // 前回使ったデッキがあればそこから編成を始められるようにする(無ければおまかせ編成)
  const last = getLastUsedDeck();
  state.deckCounts = last ? {...last.counts} : {...DEFAULT_DECK_COUNTS};
  state.currentDeckName = last ? last.name : '';
  renderDeckBuilder();
...
$('btn-start-saved').onclick = () => {
  const last = getLastUsedDeck();
  if(!last){ toast('保存されたデッキがありません。まずデッキ編成画面で保存してください'); return; }
  const ids = countsToDeckIds(last.counts);
...
// タイトルから直接デッキ編成画面を開く(対戦を始めずに編集/保存だけしたい時用)
$('btn-deck-edit').onclick = () => {
  initAudio();
  NET.mode = 'local';
  const last = getLastUsedDeck();
  state.deckCounts = last ? {...last.counts} : {...DEFAULT_DECK_COUNTS};
...
$('btn-deck-save').onclick = () => {
  ...
  const store = upsertSavedDeck(name, state.deckCounts);
  if(store){
...
$('btn-deck-delete').onclick = () => {
  if(!state.currentDeckName) return;
  const name = state.currentDeckName;
...
$('deck-select').onchange = () => {
  ...
  const store = loadDeckStore();
  const found = store.decks.find(d=>d.name===name);
  if(!found) return;
  state.deckCounts = {...found.counts};
  state.currentDeckName = name;
  renderDeckBuilder();
};

// 対戦を始めずにタイトルへ戻る(対人戦の途中であれば接続もきちんと片付ける)
$('btn-deck-back').onclick = () => {
  if(NET.mode==='host' || NET.mode==='guest' || NET.mode==='spectator'){
    pvpResetToTitle();
  }else{
    show('title-screen');
  }
