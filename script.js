...
function renderField(targetId, owner, enemy=false){
  const rows = [
    {label:'1列目：攻撃エリア', start:FRONT_START, isBack:false},
    {label:'2列目：召喚エリア', start:BACK_START, isBack:true}
  ];
  ...
  $(targetId).innerHTML = rows.map(row=>{
    const cells = Array.from({length:FIELD_LANES},(_,lane)=>{
      ...
      return `<div class="${cls}${waiting?' back-safe':''}" data-slot="${index}">${renderCard(card)}${waiting?'<div class="safe-label">召喚エリア(攻撃不可・攻撃されない)</div>':''}</div>`;
    }).join('');
    return `<div class="field-row"><div class="row-label">${row.label}</div><div class="lanes">${cells}</div></div>`;
  }).join('');
}
function renderHand(){
...
function endTurn(){
  ...
  // ターン終了時、2列目のモンスターを(1列目が空いていれば)1列目へ移動させてから攻撃処理を行う
...
function runEnemyTurn(){
  ...
  // 敵ターン終了時、2列目のモンスターを1列目へ移動させてから攻撃処理を行う
  advanceRow(state.enemy, '敵');
  render();
